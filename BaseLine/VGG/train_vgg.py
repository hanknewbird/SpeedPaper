...
if not os.path.exists(log_dir):  # 判断文件夹是否存在
...
if __name__ == "__main__":
    ...
    path_state_dict = os.path.join(BASE_DIR, "", "file", "vgg16-397923af.pth")  # 以VGG16的预训练模型参数路径

    num_classes = 2    # 定义类别
    MAX_EPOCH = 3      # 跑多少轮
    BATCH_SIZE = 256   # 每次载入多少图片
    LR = 0.001         # 学习率
    ...
    for epoch in range(start_epoch + 1, MAX_EPOCH):  # 开始训练模型
        ...
        for i, data in enumerate(train_loader):  # 对训练集进行迭代,i为第几次,data为数据
            ...
            if (i+1) % log_interval == 0:  # 是否打印日志
                ...
                print(f"Training:Epoch[{epoch:0>3}/{MAX_EPOCH:0>3}] Iteration[{i+1:0>3}/{len(train_loader):0>3}] Loss: {loss_mean:.4f} Acc:{correct / total:.2%} lr:{scheduler.get_last_lr()}")
                loss_mean = 0.
        ...
        if (epoch+1) % val_interval == 0:
            ...
            with torch.no_grad():  # 模型不需要梯度更新
                ...
                print(f"Valid:\t Epoch[{epoch:0>3}/{MAX_EPOCH:0>3}] Iteration[{j+1:0>3}/{len(valid_loader):0>3}] Loss: {loss_val_mean:.4f} Acc:{correct_val / total_val:.2%}")
