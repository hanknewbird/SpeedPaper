...
if __name__ == "__main__":
    ...
    train_transform = transforms.Compose([          # 定义图片的预训练方式
        ...
        transforms.RandomCrop(32, padding=4),   # 随机裁剪为(32,32)
    ...
    resnet_model = ResNeXt29_2x64d()  # 获取预训练模型

    resnet_model.to(device)  # 推至运算设备上
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, gamma=0.1, milestones=milestones)  # 设置学习率下降策略
    ...
    print(f"done！！"
          f"{datetime.strftime(datetime.now(), '%m-%d_%H-%M')}, "
