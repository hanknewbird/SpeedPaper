...
class expansive_block(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels):
        ...
        self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=(3, 3), stride=2, padding=1, output_padding=1, dilation=1)
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print('final_layer:', final_layer.size())
        return final_layer


if __name__ == "__main__":
    import torch as t
    # 原文输入为572*572，最后输出为388*388
    rgb = t.randn(1, 3, 572, 572)
    net = UNet(3, 2)

    # 但SpeedPaper在图像分割中的数据集输入为(640,480),输出通道数为5
    # rgb = t.randn(1, 3, 640, 480)
    # net = UNet(3, 5)
